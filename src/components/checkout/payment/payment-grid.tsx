...
const PaymentGrid: React.FC<{ className?: string; theme?: 'bw' }> = ({
  ...
  useEffect(() => {
    if (settings && availableGateway) {
      console.log('+++++++++++++',availableGateway,settings?.defaultPaymentGateway,settings?.defaultPaymentGateway?.toUpperCase() as PaymentGateway)
      setGateway(
  ...
  console.log('PaymentMethod',PaymentMethod,gateway)
