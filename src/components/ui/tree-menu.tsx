...
const TreeMenuItem: React.FC<TreeMenuItemProps> = ({
  ...
  // console.log('ac/tive,item.slug -------------',active,item.slug)
  ...
  function onClick() {
    ...
    if (Array.isArray(items) && !!items.length) {
      ...
      if (!isOpen) {
        // Add the category to the query if it's being opened
        navigate({ ...query, category: slug });
      } else {
        // Remove the category from the query if it's being closed
        const { category, ...rest } = query; // Remove the `category` key
        navigate(rest);
      }
    } else {
      // For leaf nodes, just navigate with the category
      navigate({ ...query, category: slug });
  ...
  return (
    <>
      ...
      >
        <button
          className={cn(
            'flex w-full items-center py-2 font-semibold outline-none transition-all ease-in-expo  focus:text-accent focus:outline-0 focus:ring-0 ltr:text-left rtl:text-right',
      ...
      <AnimatePresence initial={false}>
        {Array.isArray(items) && isOpen ? (
          <TreeChildItem items={items} depth={depth} />
        ) : null}
      </AnimatePresence>
    </>
  );
...
const TreeChildItem:React.FC<any> = ({
  items,depth
}) => {
          ...
          {items.map((currentItem: any) => {
            ...
            return (
              <TreeMenuItem
                ...
                item={currentItem}
                depth={childDepth}
                className={cn('text-sm  ltr:ml-5 rtl:mr-5')}
              />
            );
          })}
        </motion.ul>
