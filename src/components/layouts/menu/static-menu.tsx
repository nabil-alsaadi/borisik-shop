    ...
    <>
      {headerLinks?.slice(0, 3)?.map(({ href, label, icon }) => (
        <li key={`${href}${label}`}>
          <Link
            href={href}
            className="flex items-center font-semibold text-heading no-underline transition duration-200 hover:text-accent focus:text-accent heade-menu-a"
          >
      ...
      <li className="menuItem group relative mx-3 cursor-pointer py-3 xl:mx-4">
        <div className="flex items-center gap-2 group-hover:text-accent">
          <span className="text-brand-dark group-hover:text-brand relative inline-flex items-center py-2 font-semibold rtl:left-0 heade-menu-a">
            {t('text-pages')}
          </span>
          <ArrowDownIcon className="mt-1 heade-menu-a" />
        </div>
